...
My questions:
OK, reading through the schema's the currencies looks good. The FX rates. I have a question on I will first statement is the right type. Let's just keep spot and budget and get rid of ocean I've been told by finance that the spot will be the affordable rate and then they do need a budget rate for like beginning of the year type stuff for how to report to the board or investors. I'm curious about the CAD pricing rebased confirm override in the product types I don't see any reference diffusion ID that seems to be our main unique identifier here, but I think it might be handled in the product catalogue. I see servers 01_servers.CSV is the extracted file but I don't see any fusion ID reference to it we have access to the dim product attributes for our MSSQL database so is there any intention of merging those two before we put the data into the post grass or the super bass database? Also there is the host in the server_specs table I'm not quite sure what that really means. I think it's kind of an add-on to know that we can do an ESSI version and I think that what's important to note here is we probably need to have some additional table with ESXI compatibility ESXIX as in Xerox that shows what processors are compatible with up to what version of ESSI so that's something when you think about question